  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while (true) {
        ...
        byte[] bufferEntrada = new byte[1024]; // debemos establecer la longitud del datagrama (si el paquete excede la
                                               // longitud, se divide)
        // Ahora vamos a crear un datagrama para recibir los datos que el cliente envie
        DatagramPacket datagrama = new DatagramPacket(bufferEntrada, bufferEntrada.length); // debemos pasar el buffer
                                                                                            // de entrada y su longitud
        // El servidor va a estar esperando el mensaje del cliente
        socket.receive(datagrama);

        HiloCliente hilo = new HiloCliente(socket, datagrama); // pasamos el socket y datagrama para que sepa donde
